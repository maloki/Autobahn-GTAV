...
namespace HighBeam
{
    public static class ZoneCreatorOld
    {
        ...
        private static Stopwatch addPathStopwatch = new Stopwatch();
        ...
        public static void RunZoneCreator()
        {
            ...
            if (isCreatorOn)
            {
                if (addPathStopwatch.ElapsedMilliseconds > 10)
                {
                    PathCreator();
                    addPathStopwatch = new Stopwatch();
                }
                if (!addPathStopwatch.IsRunning)
                {
                    addPathStopwatch.Start();
                }
                UI.ShowSubtitle(pathList.Count.ToString());
            }
        ...
        private static void SavePathToFile()
        {
            ...
            { 
                tsw.Write($@"{p.Position.X},{p.Position.Y},{p.Position.Z}_{p.Direction.ToString().Replace(',', '.')};");
            }
