    ...
    public static class MoveCar
    {
        ...
        public static bool CheckIfCarIsToDelete(GeneralCar car)
        {
            ...
            if (CurrentZone.Name != null)
            {
                ...
                if (car.Vehicle.Position.Z - CurrentZone.ZoneBoundary.ZCoord > 2f && !CurrentZone.HasHill)
                    toDelete = true;
            }
            if ((distBetweenPlayer > 1200f) || zoneEnd.DistanceTo(car.Vehicle.Position) < 50 || toDelete || !isInZone || car.Vehicle.HasCollidedWithAnything)
            {
                ...
                {
                    if (car.Stats.isLeftLane)
                    {
                        carsLeftLaneCount = carsLeftLaneCount - 1;
                    }
                    else
                    {
                        carsRightLaneCount = carsRightLaneCount - 1;
                    }
        ...
        public static void AdjustCarSpeed(GeneralCar car)
        {
            if (car.Stats.deccelerate)
            {
                ...
                else
                {
                    ...
                    var bf = 0f;
                    var speedDif = (Math.Round(car.Stats.Speed, 0) - Math.Round(car.Stats.adjustSpeedTo, 0));
                    bf = 0.23f; // (float)(Math.Round(speedDif / 130, 2));
            ...
            {
                double carSpeed = car.Vehicle.Speed * 3.6;
                car.Vehicle.BrakeLightsOn = false;
                car.Stats.deccelerate = false;
                ...
                {
                    car.Stats.Speed += GetAcceleration(carSpeed);
                }
            }
        }

        private static float GetAcceleration(double speed)
        {
            if(speed < 100)
            {
                return 0.14f;
